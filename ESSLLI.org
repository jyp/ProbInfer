...
Represent a probabilistic program as a list of pairs: \((outcome,probability)\).

Then:

#+begin_src Haskell
⟦return k⟧    =   [(k,1)]
⟦x <- t; u⟧   =   [(y,p×q) | (x,p) ∈ ⟦t⟧, (y,q) ∈ ⟦u⟧]
⟦sample d⟧    =   [(PDF d x,x) | x ∈ Ω]
...
** Structure of the probabilistic program
