...
- Clone the repository: ~git clone TODO; cd TODO~ (The lecture notes are in ~ESSLLI.org~)
...
* Inference under uncertainty: Problem description
  ...
  involves reasoning with and about incomplete, uncertain, vague
...
** Logic as the means of categorical judgments/inferences
   - simple Aristotelian reasoning
   - limitations of Aristotelian reasoning
...
*** First-Order Predicate Logic 
  ...
  be seen as a heir of the Aristotelian syllogisms. 
  Introduced by Gottlob Frege in the 19th Century, FOL makes use of:
  - logcial connectives (&, ∨, →, ¬)
  ...
  - quantifiers ∀x and ∃x where x is a variable
  
  A ~term~ is obtained by applying n-ary function to constants,
  variables, and/or other terms, which are n in total.
  An ~atomic formula~ is an n_ary predicate whose all places are occupied by
  terms. 
  A ~literal (aka molecule)~ is a formula such as P(t1,...,tn) or ¬P(t1,...,tn) 
  where P is an n-ary predicate symbol and t1,...,tn are terms. 
  A ~formula~ is either:
                       - a literal 
                       - application of logical connectives to formulae
                       _ application of a quantifier to a formula: ∀xA or ∃xA
...
** Inferences that are non-categorical (non-logical inferences)
  ...
  - Teddy is 18 months old.                          ⊨? Teddy weights less than 15 kg.
  - ⊨? Teddy is a bear.
  ...
  Here, we limit our discussion to problems which require no external
  knowledge about the world.  This means in practice that we will
  focus on problems which are /similar/ to the Aristotelian square,
...
** A type of non-logical inference: Probabilistic inference
  ...
  Our focus is on sentences which are /similar/ to universally
  ...
  - Dogs usually are friendly. Pluto is a dog.              $⊨$? Pluto is friendly.
  - Many cats sleep.           Tom is a cat.                $⊨$? Tom sleeps.
  - Mice are not lovely.       Micky is an atypical mouse.  $⊨$? Micky is not lovely.

  One of the main questions that we may ask is the following: Are the
  rules governing reasoning under uncertainty part of the same system
  ...
  natural languages (like English) with the help of morpho-syntactic
  and/or lexical apparatus.
...
*** Generalized Quantifiers 
   ...
   - Dracula is a vampire. Is he sleeping? (Probably yes, but one cannot claim it with full certainty.)

     In this way, we can construct various quantified propositions
     which would carry probabilistic meaning. One way of thinking
...
*** Graded Adjectives and Comparatives
    ...
    NBA players are taller than most people in US.
    Few people are NBA players. 
    Muggsy Bogues is an NBA player.
    ---------------------------------------------
    $⊨$? Muggsy Bogues is taller than most people in US.
...
*** Probability distributions (1)
 The basic building block to describe possible worlds (probabilistic
 programs) are probability distributions. We will list and discuss
 some of them.
...
*** Expected truth value (aka “Probability”)
 ...
 The program representing situations where the sum of dice is \(> 6\) is :

#+begin: example-src :filename "DiceExample.hs" :defn twoDieAbove6
twoDieAbove6 = do
  d₁ <- die
  d₂ <- die
  observe (d₁ + d₂ > 8)
...
\(𝔼_{[(x,y)∈twoDieAbove9]}(Indicator(x×y > 20))\)
...
exampleSocrates0 = do
  ...
  observe (every man mortal)  -- filter premiss
  socrates ← sampleInd        -- declare individual ("for some random ...")
  observe (man socrates)      -- filter premiss
  return (mortal socrates)    -- conclusion, interpreted by Montagovian semantics
#+end:
...
* Computations: MCMC, Variational Inference
So far, we can:
...
BUT:
- these formulas involve integrals which are typically not easy to
  compute (when involving non-trivial spaces).
...
To evaluate \(𝔼_{(x∈t)} (x)\):
 - ~n := 0; q := 0~
 - repeat:
   - take a random \(x\) by sampling from \(t\)
   - ~n := n + 1~
   - ~q := q + x~
...
- assume set of states \(S\), and a starting state s₀.
...
- the transition function is such that, as much as possible, \(s(x,y)
  > s(x,y')\) if \(P_t(y) > P_t(y')\).
- note that this is not an easy thing to define when there are many
  variables (also, with if statements (and |observe|), the existence
  of variables depend on the value of others.)
...
Assume a set of parameters \(θ\) for your model. How to take into
account an observation $e$? We do:

Together with the model, the parameters yield an estimated probability for \(e\) to occur:

\(P(e ∣ θ)\).

Using Bayesian reasoning, we can update \(θ\):

\(P(θ ∣ e) ∝ P(e∣θ) × P(θ)\)

Question: what is the normalisation factor?

But this requires keeping track of the whole *distribution* of possible parameters θ.

An alternative is to keep a single (the “best”) value of θ, and take
into account events by gradient descent. To take $e$ into account, compute
...
Any mixture of the above is thinkable. (Sample several \(θ\) and do
gradient decsent for each of them, etc.)
...
[[file:~/repo/gu/PNLS/Frontend/MCMC.hs::mcmc][Link]]

** More Probabilistic programming packages
- STAN: https://mc-stan.org/
- WebPPL http://dippl.org/
- And many, many, many papers about real and idealised probabilistic
...
* TODOs
  - move this document and source code to own repository
